...
/**
 * Optional `assignedFrom` and `assignedTo` as YYYY-MM-DD (inclusive of both days, local time).
 * Returns null for all-time (no range) or if invalid.
 */
export function getLeadAssignedDateRange(
  searchParams: URLSearchParams
): { gte: Date; lte: Date } | null {
  const from = searchParams.get('assignedFrom')
  const to = searchParams.get('assignedTo')
  if (!from || !to) return null
  const gte = new Date(from + 'T00:00:00.000')
  const lte = new Date(to + 'T23:59:59.999')
  if (Number.isNaN(gte.getTime()) || Number.isNaN(lte.getTime()) || gte > lte) return null
